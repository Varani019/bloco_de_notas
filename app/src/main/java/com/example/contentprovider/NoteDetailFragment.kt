...
import com.example.contentprovider.databese.NotesDatabaseHelper
import com.example.contentprovider.databese.NotesDatabaseHelper.Companion.DESCRIPTION_NOTE
...
class NoteDetailFragment: DialogFragment(), DialogInterface.OnCancelListener {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        val view = activity?.layoutInflater?.inflate(R.layout.note_detall, null)
        ...
        if(arguments != null && arguments?.getLong(EXTRA_ID) != 0L ){
            ...
            if(cursor?.moveToNext() as Boolean){
                ...
                noteEditDescription.setText(cursor.getString(cursor.getColumnIndex(DESCRIPTION_NOTE)))
            }
            cursor.close()
        }
